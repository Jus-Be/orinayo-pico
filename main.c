...
#define REG_ENC_START   0x00  // Base address for inputs
#define REG_RGB_START   0x40  // Base address for RGB lights

// Complete Input Data Map (0x00 to 0x30)
typedef struct {
    int32_t counters[8];  // 0x00 to 0x1F (32 bytes)
    uint8_t buttons[8];   // 0x20 to 0x27 (8 bytes)
    uint8_t padding[8];   // 0x28 to 0x2F (8 bytes reserved space in M5 firmware)
    uint8_t slide_switch; // 0x30 (1 byte: 0 = Off/Left, 1 = On/Right)
} __attribute__((packed)) encoder_inputs_t;

// Output Layout helper for an individual LED
typedef struct {
    uint8_t r;
    uint8_t g;
    uint8_t b;
...
// Complete RGB Structure for one single-burst write payload
typedef struct {
    uint8_t reg_start;          // Targets the address 0x40
    rgb_color_t leds[8];        // 8 LEDs * 3 bytes (24 bytes)
} __attribute__((packed)) rgb_payload_t;
...
// Function to write color values to all 8 LEDs in one sweep
void update_all_leds(rgb_payload_t *payload) {
    payload->reg_start = REG_RGB_START;
    // Send 25 bytes total (1 byte register address + 24 bytes RGB matrix data)
    i2c_write_blocking(I2C_PORT, ENCODER_ADDR, (uint8_t*)payload, sizeof(rgb_payload_t), false);
}
            ...
            if (bytes_read == sizeof(encoder_inputs_t)) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if (inputs.buttons[i] == 1) {
                        ...
                        led_data.leds[i].g = 0;
                        led_data.leds[i].b = 0;
                    } else if (inputs.slide_switch == 1) {
                        led_data.leds[i].r = 0;
                        led_data.leds[i].g = 0;
                        led_data.leds[i].b = 30;  // Soft Blue
                    } else {
                        led_data.leds[i].r = 0;
                        led_data.leds[i].g = 30;  // Soft Green
                        led_data.leds[i].b = 0;
                    }
                }
